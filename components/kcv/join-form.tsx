  ...
  return (
    <NeoCard className="w-full p-5 sm:p-7">
      ...
      <form onSubmit={submit} className="space-y-4">
        <label className="block"><span className="mb-2 block text-sm font-bold">Team name</span><input className="neo-input" value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="The Pixel Seekers" autoComplete="off" maxLength={40} required /></label>
        <label className="block"><span className="mb-2 block text-sm font-bold">Room code</span><input className="neo-input uppercase tracking-[.22em]" value={roomCode} onChange={(e) => setRoomCode(e.target.value.toUpperCase())} placeholder="KCV26" autoComplete="off" maxLength={12} required /></label>
