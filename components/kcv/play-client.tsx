...
type State = {
  ...
  submission: null | { id: string; evaluationStatus: string; decision: "accepted" | "rejected" | null; detectedObject: string | null; reason: string | null; confidence: number | null; attemptNumber: number; clueText: string | null; imageUrl: string | null };
...
function FreezeScreen({ onContinue, busy }: { onContinue: () => void; busy: boolean }) {
  return <main className="flex min-h-dvh bg-white px-5 py-7 sm:items-center sm:px-8 sm:py-12"><div className="mx-auto w-full max-w-md"><StatusBadge tone="orange"><Trophy size={13} /> Leaderboard update</StatusBadge><h1 className="display mt-5 text-[3.25rem] leading-[.9] sm:text-6xl">The Leaderboard is Frozen</h1><NeoCard className="mt-6 bg-kcv-yellow p-5 sm:mt-8 sm:p-6"><p className="font-bold leading-relaxed">You can still complete more clues and earn points, but rankings will no longer be shown. The winner will be announced after the game ends.</p></NeoCard><NeoButton onClick={onContinue} disabled={busy} className="mt-7 w-full sm:mt-9">Keep hunting <ChevronRight /></NeoButton></div></main>;
}
...
function ResultScreen({ state, accepted, onAction, busy }: { state: State; accepted: boolean; onAction: () => void; busy: boolean }) { const submission = state.submission; const clueText = submission?.clueText ?? state.assignment?.clue.text ?? "this clue"; return <main className="participant-page"><div className="mx-auto w-full max-w-md"><div className="text-center"><div className={`mx-auto flex h-20 w-20 items-center justify-center rounded-full border-2 border-black ${accepted ? "bg-kcv-green" : "bg-kcv-pink"}`}>{accepted ? <Check size={40} strokeWidth={3} /> : <CircleAlert size={38} />}</div><p className="kicker mt-6">{accepted ? "Clue completed" : "Not quite yet"}</p><h1 className="display mt-3 text-5xl">{accepted ? "That’s a match!" : "Give it another shot"}</h1><p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-zinc-600">{accepted ? "Yes, it’s " : "I don’t think it’s "}<b className="text-zinc-900">{clueText}</b></p></div>{submission?.imageUrl && <div className="mt-8 overflow-hidden rounded-[2rem] border-2 border-black aspect-[4/3]"><img src={submission.imageUrl} alt="Evaluated submission" className="h-full w-full object-cover" /></div>}<NeoCard className={`mt-6 p-6 ${accepted ? "bg-kcv-green" : "bg-kcv-pink"}`}><dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm"><dt className="font-black">Detected</dt><dd>{submission?.detectedObject || "Unclear"}</dd><dt className="font-black">Reason</dt><dd>{submission?.reason || "The evaluator could not verify the object."}</dd><dt className="font-black">Confidence</dt><dd>{submission?.confidence == null ? "—" : `${Math.round(submission.confidence * 100)}%`}</dd><dt className="font-black">Attempt</dt><dd>{submission?.attemptNumber ?? "—"}</dd>{accepted && <><dt className="font-black">Total score</dt><dd>{state.team.totalScore} points</dd></>}</dl></NeoCard><NeoButton onClick={onAction} disabled={busy} className="mt-8 w-full">{accepted ? (state.phase === "active_leaderboard_visible" ? "See the leaderboard" : "Next clue") : "Try again"} <ChevronRight /></NeoButton></div></main>; }

function LeaderboardScreen({ state, onContinue, busy }: { state: State; onContinue: () => void; busy: boolean }) { const board = state.leaderboard ?? []; const own = board.find((row) => row.id === state.team.id); return <main className="participant-page"><div className="mx-auto w-full max-w-md"><p className="kicker">Live until the freeze</p><div className="mt-3 flex items-end justify-between"><h1 className="display text-5xl">Leaderboard</h1>{own && <StatusBadge tone="blue">#{own.rank}</StatusBadge>}</div><div className="mt-8 space-y-3">{board.map((team) => <NeoCard key={team.id} className={`flex items-center gap-4 px-5 py-4 ${team.id === state.team.id ? "bg-kcv-yellow" : "bg-white"}`}><span className="display w-8 text-2xl">{team.rank}</span><div className="min-w-0 flex-1"><p className="truncate font-black">{team.name}</p><p className="text-xs text-zinc-500">{team.completedClueCount} clues · {team.totalAttemptCount} attempts</p></div><b className="display text-xl">{team.totalScore}</b></NeoCard>)}</div><NeoButton onClick={onContinue} disabled={busy} className="mt-9 w-full">Continue to next clue <ChevronRight /></NeoButton></div></main>; }
